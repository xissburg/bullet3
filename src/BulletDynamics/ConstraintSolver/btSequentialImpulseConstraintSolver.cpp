		...
		if (infoGlobal.m_solverMode & SOLVER_USE_WARMSTARTING)
		{
			...
			if (rb0) {
				if (bodyA->m_useSplitSpin) {
					bodyA->internalApplyImpulseWithSpinSplit(frictionConstraint1.m_contactNormal1*rb0->getInvMass()*rb0->getLinearFactor(),frictionConstraint1.m_angularComponentA,frictionConstraint1.m_appliedImpulse);
				} else {
			...
			if (rb1) {
				if (bodyB->m_useSplitSpin) {
					bodyB->internalApplyImpulseWithSpinSplit(-frictionConstraint1.m_contactNormal2*rb1->getInvMass()*rb1->getLinearFactor(),-frictionConstraint1.m_angularComponentB,-(btScalar)frictionConstraint1.m_appliedImpulse);
				} else {
    ...
    )
{
	...
	btSolverBody* bodyCPtr = &m_tmpSolverBodyPool[solverBodyIdC];
...
void btSequentialImpulseConstraintSolver::convertJoints(btTypedConstraint** constraints,int numConstraints,const btContactSolverInfo& infoGlobal)
{
	...
	for (int i=0;i<numConstraints;i++)
	{
		...
		if (info1.m_numConstraintRows)
		{
			...
			int solverBodyIdC = getOrInitSolverBody(rbC,infoGlobal.m_timeStep);

            convertJoint(currentConstraintRow, constraint, info1, solverBodyIdA, solverBodyIdB, solverBodyIdC, infoGlobal);
        }
...
btScalar btSequentialImpulseConstraintSolver::solveSingleIteration(int iteration, btCollisionObject** /*bodies */,int /*numBodies*/,btPersistentManifold** /*manifoldPtr*/, int /*numManifolds*/,btTypedConstraint** constraints,int numConstraints,const btContactSolverInfo& infoGlobal,btIDebugDraw* /*debugDrawer*/)
{
	...
	for (int j=0;j<m_tmpSolverNonContactConstraintPool.size();j++)
	{
		...
		if (iteration < constraint.m_overrideNumSolverIterations)
		{
			...
			btSolverBody& bodyC = m_tmpSolverBodyPool[constraint.m_solverBodyIdC];
			...
			if (constraint.m_flags & 
				...
				residual = resolveSingleConstraintRowGenericSplitSpin(bodyA,bodyB,bodyC,constraint);
			}
			else {
				residual = resolveSingleConstraintRowGeneric(bodyA,bodyB,bodyC,constraint);
			}
